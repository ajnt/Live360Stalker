...
namespace Examplinvi
{
    ...
    class Program
    {
        ...
        public static Dictionary<string, int> DeserializeDict(string sXML)
        {
            ...
            {
                d.Add(n.ChildNodes[0].InnerText.ToString(), int.Parse(n.ChildNodes[1].InnerText));
            }
            return d;
        }
        private static void WriteStatusItem(string sMsg)
        {
            Console.WriteLine(sMsg);
            File.AppendAllText("output.txt", sMsg + System.Environment.NewLine);
        }

        static void Main()
        {
            // Load all the players
            Dictionary<string, int> dPlayersAndScores = new Dictionary<string, int>()
            {
                { "maskeddev", 0},
                { "ajnt", 0},
                { "gorric89", 0},
            ...
            try
            {
                string sXML = File.ReadAllText("scores.xml");
                dPlayersAndScores = DeserializeDict(sXML);
            }
            catch (Exception e)
            {
            ...
            stream.AddTrack("#Live360Stalker");
            stream.AddTrack("#Live360stalker");
            stream.AddTrack("#live360stalker");
            stream.AddTrack("#live360Stalker");

            WriteStatusItem(DateTime.Now.ToString() + " Stream tracking running");
            

            stream.MatchingTweetReceived += (sender, args) =>
            {
                var tweet = args.Tweet;
                string sAuthor = tweet.CreatedBy.ScreenName.ToLower();
                if (!tweet.IsRetweet && tweet.UserMentions.Count > 0 && tweet.Media.Count > 0)
                {

                    foreach (UserMentionEntity u in tweet.UserMentions)
                    {
                        if (u.ScreenName.ToLower() != "live360events")
                        {
                            if (dPlayersAndScores.ContainsKey(sAuthor))
                            {
                                dPlayersAndScores[sAuthor] = dPlayersAndScores[sAuthor] + 1;
                            }
                            else
                            {
                                dPlayersAndScores.Add(sAuthor, 1);
                            }
                        }
                    }

                    string sWinner = dPlayersAndScores.OrderByDescending(x => x.Value).First().Key;
                    int iWinnerScore = dPlayersAndScores.OrderByDescending(x => x.Value).First().Value;
                    int iAuthorScore = dPlayersAndScores.Where(x => x.Key == sAuthor).First().Value;
                    string text = "Well done @" + sAuthor + ", you have " + iAuthorScore.ToString() + " points. Current lead #Live360Stalker is @" + sWinner + " with a score of " + iWinnerScore.ToString();

                    WriteStatusItem(DateTime.Now.ToString() + " " + tweet.Text);
                    
                    WriteStatusItem(tweet.Text);
                    WriteStatusItem(text);
                    Examples.Tweet_PublishTweetInReplyToAnotherTweet(text, tweet.Id);
                    File.WriteAllText("scores.xml", SerializeDict(dPlayersAndScores));
                    WriteStatusItem(DateTime.Now.ToString() + " ");
                }
                else
                {
                    WriteStatusItem(DateTime.Now.ToString() + " Tweet ignored: " + tweet.Text);
                    WriteStatusItem(DateTime.Now.ToString() + " Tweet is retweet? " + tweet.IsRetweet);
                    WriteStatusItem(DateTime.Now.ToString() + " Tweet user mentions " + tweet.UserMentions.Count);
                    WriteStatusItem(DateTime.Now.ToString() + " Tweet media count " + tweet.Media.Count);
                    WriteStatusItem(DateTime.Now.ToString() + " ");
                    string sTweetIgnoredReason = "";

                    if(!tweet.Text.Contains("Well done") && !tweet.Text.Contains("Current lead")) { 
                    if (tweet.Media.Count == 0) { sTweetIgnoredReason += "No photo attached."; }
                    //if (tweet.IsRetweet) { sTweetIgnoredReason += "Is a retweet."; }
                    if (tweet.UserMentions.Count == 0) { sTweetIgnoredReason += "No users mentioned using @."; }
                    if (sTweetIgnoredReason != "")
                    {
                        Examples.Tweet_PublishTweetInReplyToAnotherTweet("Tweet from " + sAuthor + " not scored, because " + sTweetIgnoredReason + " cc @ajnt", tweet.Id);
                    }
                    }
                }
            };
            ...
            WriteStatusItem(@"END");
            Examples.Tweet_PublishTweet("@ajnt Live360StalkerBot has finished running, please restart");
            Console.ReadLine();
        }
    ...
    static class Examples
    {
        ...
        public static ITwitterCredentials CredentialsCreator_WithCaptcha_StepByStep(string consumerKey, string consumerSecret)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Go on : {0}", url);
            Console.WriteLine(DateTime.Now.ToString() + " Enter the captch : ");
            var captcha = Console.ReadLine();

            var newCredentials = CredentialsCreator.GetCredentialsFromVerifierCode(captcha, applicationCredentials);
            Console.WriteLine(DateTime.Now.ToString() + " Access Token = {0}", newCredentials.AccessToken);
            Console.WriteLine(DateTime.Now.ToString() + " Access Token Secret = {0}", newCredentials.AccessTokenSecret);
        ...
        public static ITwitterCredentials CredentialsCreator_CreateFromRedirectedCallbackURL_StepByStep(string consumerKey, string consumerSecret)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Go on : {0}", url);
            Console.WriteLine(DateTime.Now.ToString() + " When redirected to your website copy and paste the URL: ");
        ...
        public static ITwitterCredentials CredentialsCreator_CreateFromRedirectedVerifierCode_StepByStep(string consumerKey, string consumerSecret)
        {
            ...
            var url = CredentialsCreator.GetAuthorizationURL(applicationCredentials, "https://tweetinvi.codeplex.com");
            Console.WriteLine(DateTime.Now.ToString() + " Go on : {0}", url);
            Console.WriteLine(DateTime.Now.ToString() + " When redirected to your website copy and paste the value of the oauth_verifier : ");
        ...
        public static void Tweet_PublishTweetInReplyToAnotherTweet(string text, long tweetIdtoReplyTo)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Publish success? {0}", tweet != null);
        }
        ...
        public static void Tweet_PublishRetweet(long tweetId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " You retweeted : '{0}'", retweet.Text);
        }
        ...
        public static void Tweet_GetRetweets(long tweetId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} retweeted : '{1}'", firstRetweeter.Name, originalTweet.Text);
        }
        ...
        public static void Tweet_GenerateOEmbedTweet()
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Oembed tweet url : {0}", oembedTweet.URL);
        ...
        public static void Tweet_Destroy()
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Has the tweet destroyed? {0}", tweetDestroyed);
        }
        ...
        public static void Tweet_SetTweetAsFavorite(long tweetId)
        {
            var tweet = Tweet.GetTweet(tweetId);
            tweet.Favourite();
            Console.WriteLine(DateTime.Now.ToString() + " Is tweet now favourite? -> {0}", tweet.Favourited);
        }
        ...
        public static void User_GetFriendIds(string userName)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} has {1} friends, here are some of them :", user.Name, user.FriendsCount);
            foreach (var friendId in friendIds)
            {
                Console.WriteLine(DateTime.Now.ToString() + " - {0}", friendId);
            }
        }
        ...
        public static void User_GetFriends(string userName)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " - {0}", friend.Name);
            }
        }
        ...
        public static void User_GetFollowerIds(string userName)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} has {1} followers, here are some of them :", user.Name, user.FollowersCount);
            foreach (var followerId in followerIds)
            {
                Console.WriteLine(DateTime.Now.ToString() + " - {0}", followerId);
            }
        }
        ...
        public static void User_GetFollowers(string userName)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " - {0}", follower.Name);
            }
        }
        ...
        public static void User_GetRelationshipBetween(string sourceUserName, string targetUsername)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " You are{0} following {1}", relationship.Following ? "" : " not", targetUsername);
            Console.WriteLine(DateTime.Now.ToString() + " You are{0} being followed by {1}", relationship.FollowedBy ? "" : " not", targetUsername);
        }
        ...
        public static void User_GetFavorites(string userName)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} has {1} favorites, here are some of them :", user.Name, user.FavouritesCount);
            foreach (var favoriteTweet in favorites)
            {
                Console.WriteLine(DateTime.Now.ToString() + " - {0}", favoriteTweet.Text);
            }
        }
        ...
        public static void User_BlockUser(string userName)
        {
            ...
            if (user.BlockUser())
            {
                Console.WriteLine(DateTime.Now.ToString() + " {0} has been blocked.", userName);
            }
            else
            {
                Console.WriteLine(DateTime.Now.ToString() + " {0} has not been blocked.", userName);
            }
        }
        ...
        public static void LoggedUser_GetIncomingRequests()
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " {0} wants to follow you!", user.Name);
            }
        }
        ...
        public static void LoggedUser_GetOutgoingRequests()
        {
            ...
            foreach (var user in usersRequestingFriendship)
            {
                Console.WriteLine(DateTime.Now.ToString() + " You sent a request to follow {0}!", user.Name);
            }
        }
        ...
        public static void LoggedUser_FollowUser(string userName)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " You have successfully sent a request to follow {0}", userToFollow.Name);
            }
        }
        ...
        public static void LoggedUser_UnFollowUser(string userName)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " You are not following {0} anymore", userToFollow.Name);
            }
        }
        ...
        public static void LoggedUser_UpdateFollowAuthorizationsForUser(string userName)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " Authorizations updated");
            }
        }
        ...
        public static void LoggedUser_GetLatestReceivedMessages()
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Messages Received : ");
            foreach (var message in messages)
            {
                Console.WriteLine(DateTime.Now.ToString() + " - '{0}'", message.Text);
            }
        }
        ...
        public static void LoggedUser_GetAccountSettings()
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} uses lang : {1}", settings.ScreenName, settings.Language);
        }
        ...
        public static void Stream_FilteredStreamExample()
        {
            ...
            {
                var tweet = args.Tweet;
                Console.WriteLine(DateTime.Now.ToString() + " {0} was detected between the following tracked locations:", tweet.Id);
                ...
                foreach (var matchingLocation in matchingLocations)
                {
                    ...
                    Console.WriteLine(DateTime.Now.ToString() + " ({0}, {1})", matchingLocation.Coordinate2.Latitude, matchingLocation.Coordinate2.Longitude);
                }
            };
        ...
        public static void Stream_UserStreamExample()
        {
            ...
            userStream.LoggedUserProfileUpdated += (sender, args) =>
            {
                ...
                Console.WriteLine(DateTime.Now.ToString() + " Logged user '{0}' has been updated!", newLoggedUser.Name);
            };
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " Application {0} had its access revoked!", args.Info.ApplicationName);
            };
        ...
        public static void EventsRelatedWithTweetAndFavourite(IUserStream userStream)
        {
            ...
            userStream.TweetFavouritedByAnyone += (sender, args) =>
            {
                ...
                Console.WriteLine(DateTime.Now.ToString() + " User '{0}' favourited tweet '{1}'", userWhoFavouritedTheTweet.Name, tweet.Id);
            };
            ...
            userStream.TweetFavouritedByMe += (sender, args) =>
            {
                ...
                Console.WriteLine(DateTime.Now.ToString() + " Logged User '{0}' favourited tweet '{1}'", loggedUser.Name, tweet.Id);
            };
            ...
            userStream.TweetUnFavouritedByAnyone += (sender, args) =>
            {
                var tweet = args.Tweet;
                var userWhoFavouritedTheTweet = args.FavouritingUser;
                Console.WriteLine(DateTime.Now.ToString() + " User '{0}' unfavourited tweet '{1}'", userWhoFavouritedTheTweet.Name, tweet.Id);
            };

            userStream.TweetUnFavouritedByMe += (sender, args) =>
            {
                Console.WriteLine(DateTime.Now.ToString() + " Tweet unfavourited by me!");
            };
        ...
        public static void EventsRelatedWithLists(IUserStream userStream)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " List '{0}' created!", args.List.Name);
            };

            userStream.ListUpdated += (sender, args) =>
            {
                Console.WriteLine(DateTime.Now.ToString() + " List '{0}' updated!", args.List.Name);
            };

            userStream.ListDestroyed += (sender, args) =>
            {
                Console.WriteLine(DateTime.Now.ToString() + " List '{0}' destroyed!", args.List.Name);
            };
            ...
            userStream.LoggedUserAddedMemberToList += (sender, args) =>
            {
                ...
                var list = args.List;
                Console.WriteLine(DateTime.Now.ToString() + " You added '{0}' to the list : '{1}'", newUser.Name, list.Name);
            };
            ...
            userStream.LoggedUserAddedToListBy += (sender, args) =>
            {
                var newUser = args.User;
                var list = args.List;
                Console.WriteLine(DateTime.Now.ToString() + " You haved been added to the list '{0}' by '{1}'", list.Name, newUser.Name);
            };
            ...
            userStream.LoggedUserRemovedMemberFromList += (sender, args) =>
            {
                var newUser = args.User;
                var list = args.List;
                Console.WriteLine(DateTime.Now.ToString() + " You removed '{0}' from the list : '{1}'", newUser.Name, list.Name);
            };
            ...
            userStream.LoggedUserRemovedFromListBy += (sender, args) =>
            {
                var newUser = args.User;
                var list = args.List;
                Console.WriteLine(DateTime.Now.ToString() + " You haved been removed from the list '{0}' by '{1}'", list.Name, newUser.Name);
            };
            ...
            {
                var list = args.List;
                Console.WriteLine(DateTime.Now.ToString() + " You have subscribed to the list '{0}", list.Name);
            };
            ...
            userStream.UserSubscribedToListCreatedByMe += (sender, args) =>
            {
                var list = args.List;
                var user = args.User;
                Console.WriteLine(DateTime.Now.ToString() + " '{0}' have subscribed to your list '{1}'", user.Name, list.Name);
            };
            ...
            {
                var list = args.List;
                Console.WriteLine(DateTime.Now.ToString() + " You have unsubscribed from the list '{0}'", list.Name);
            };
            ...
            userStream.UserUnsubscribedToListCreatedByMe += (sender, args) =>
            {
                var list = args.List;
                var user = args.User;
                Console.WriteLine(DateTime.Now.ToString() + " '{0}' have unsubscribed from your list '{1}'", user.Name, list.Name);
            };
        }
        ...
        public static void Search_SimpleTweetSearch()
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " {0}", tweet.Text);
            }
        }
        ...
        public static void SavedSearch_GetSavedSearches()
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Saved Searches");
            foreach (var savedSearch in savedSearches)
            {
                Console.WriteLine(DateTime.Now.ToString() + " - {0} => {1}", savedSearch.Name, savedSearch.Query);
            }
        }
        ...
        public static void SavedSearch_CreateSavedSearch(string query)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Saved Search created as : {0}", savedSearch.Name);
        }

        public static void SavedSearch_GetSavedSearch(long searchId)
        {
            var savedSearch = SavedSearch.GetSavedSearch(searchId);
            Console.WriteLine(DateTime.Now.ToString() + " Saved searched query is : '{0}'", savedSearch.Query);
        }
        ...
        public static void SavedSearch_DestroySavedSearch(long searchId)
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " You destroyed the search successfully!");
            }
        }
        ...
        public static void Message_GetMessageFromId(long messageId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Message from {0} to {1} : {2}", message.Sender, message.Recipient, message.Text);
        }
        ...
        public static void Message_DestroyMessageFromId(long messageId)
        {
            ...
            if (message.Destroy())
            {
                Console.WriteLine(DateTime.Now.ToString() + " Message successfully destroyed!");
            }
        }
        ...
        public static void Message_PublishMessage(string text, string username)
        {
            ...
            if (message != null)
            {
                Console.WriteLine(DateTime.Now.ToString() + " Message published : {0}", message.IsMessagePublished);
            }
        }
        ...
        public static void TwitterList_GetUserOwnedLists()
        {
            ...
            ownedLists.ForEach(list => Console.WriteLine(DateTime.Now.ToString() + " - {0}", list.FullName));
        }
        ...
        public static void TwitterList_GetUserSubscribedLists()
        {
            ...
            lists.ForEach(list => Console.WriteLine(DateTime.Now.ToString() + " - {0}", list.FullName));
        }

        public static void TwitterList_GetExistingListById(long listId)
        {
            var list = TwitterList.GetExistingList(listId);
            Console.WriteLine(DateTime.Now.ToString() + " You have retrieved the list {0}", list.Name);
        }

        public static void TwitterList_CreateList()
        {
            var list = TwitterList.CreateList("plop", PrivacyMode.Public, "description");
            Console.WriteLine(DateTime.Now.ToString() + " List '{0}' has been created!", list.FullName);
        }
        ...
        public static void TwitterList_UpdateList(long listId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " List new name is : {0}", list.Name);
        }
        ...
        public static void TwitterList_DestroyList(long listId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Tweet {0} been destroyed.", hasBeenDestroyed ? "has" : "has not");
        }
        ...
        public static void TwitterList_CheckUserMembership(long userId, long listId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} is{1}a member of list {2}", userId, isUserAMember ? " " : " NOT ", listId);
        }
        ...
        public static void TwitterList_CheckUserSubscription(long userId, long listId)
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " {0} is{1}subscribed to the list {2}", userId, isUserASubscriber ? " " : " NOT ", listId);
        }
        ...
        public static void RateLimits_Track_Examples()
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " {0} is awaiting {1}ms for RateLimit to be available", args.Query, args.ResetInMilliseconds);
            };
        ...
        public static void GetCurrentCredentialsRateLimits()
        {
            ...
            Console.WriteLine(DateTime.Now.ToString() + " Remaning Requests for GetRate : {0}", tokenRateLimits.ApplicationRateLimitStatusLimit.Remaining);
            Console.WriteLine(DateTime.Now.ToString() + " Total Requests Allowed for GetRate : {0}", tokenRateLimits.ApplicationRateLimitStatusLimit.Limit);
            Console.WriteLine(DateTime.Now.ToString() + " GetRate limits will reset at : {0} local time", tokenRateLimits.ApplicationRateLimitStatusLimit.ResetDateTime.ToLongTimeString());
        }
        ...
        public static void Friendship_GetMultipleRelationships()
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " You are{0} following {1}", relationship.Following ? "" : " not", relationship.TargetScreenName);
                Console.WriteLine(DateTime.Now.ToString() + " You are{0} being followed by {1}", relationship.FollowedBy ? "" : " not", relationship.TargetScreenName);
                Console.WriteLine();
            }
        }
        ...
        public static void Friendship_GetUserRequestingFriendship()
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " {0} requested to be your friend!", friendshipRequest.Name);
            }
        }
        ...
        public static void Friendship_GetUsersYouRequestedToFollow()
        {
            ...
            {
                Console.WriteLine(DateTime.Now.ToString() + " You want to be friend with {0}.", user.Name);
            }
        }
        ...
        public static void GlobalEvents()
        {
            ...
            TweetinviEvents.QueryBeforeExecute += (sender, args) =>
            {
                Console.WriteLine(DateTime.Now.ToString() + " The query {0} is about to be executed.", args.TwitterQuery);
            };

            TweetinviEvents.QueryAfterExecute += (sender, args) =>
            {
                Console.WriteLine(DateTime.Now.ToString() + " The query {0} has just been executed.", args.TwitterQuery);
            };
        }
